...
// Datum 7 dagen terug vanaf vandaag ophalen in Y-m-d formaat
...
$sql = "SELECT spelers.naam, COALESCE(SUM(CASE WHEN matches.draw = 0 AND matches.flag IS NULL THEN 1 ELSE 0 END), 0) AS aantal_wins
        ...
        WHERE DATE(matches.datum) >= '$seven_days_ago'
...
if ($result = $mysqli->query($sql)) {
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<li>" . $positie . ". " . htmlspecialchars($row["naam"]) . " - Gewonnen potjes deze week: " . $row["aantal_wins"] . "</li>";
            $positie++;
        }
        echo "</ul>";
    } else {
        echo "Geen spelersgegevens beschikbaar voor deze week.";
    }
    $result->close();
} else {
