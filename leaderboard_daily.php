...
// Huidige datum ophalen in Y-m-d formaat
$today = date("Y-m-d");
...
$sql = "SELECT spelers.naam, COALESCE(SUM(CASE WHEN matches.draw = 0 AND matches.flag IS NULL THEN 1 ELSE 0 END), 0) AS aantal_wins
        ...
        WHERE DATE(matches.datum) = '$today'
...
if ($result = $mysqli->query($sql)) {
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<li>" . $positie . ". " . htmlspecialchars($row["naam"]) . " - Gewonnen potjes vandaag: " . $row["aantal_wins"] . "</li>";
            $positie++;
        }
        echo "</ul>";
    } else {
        echo "Geen spelersgegevens beschikbaar voor vandaag.";
    }
    $result->close();
} else {
